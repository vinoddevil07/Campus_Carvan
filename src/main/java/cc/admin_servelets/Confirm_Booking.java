...
public class Confirm_Booking extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//System.out.println( booking_ids+" " +driver_name+" " +vehiclenumber+" " +message );
		
		
		  AdminDao dao = new AdminDao();
	int status=	  dao.confirmbooking(booking_ids, driver_name, vehiclenumber, message);
	if(status>0) {
    	request.setAttribute("msg", UserMessages.Confirm_Booking);
		RequestDispatcher rd = request.getRequestDispatcher("/admin/view_confirm_booking.jsp"); // ONLY TAKE RELATIVE PATH
